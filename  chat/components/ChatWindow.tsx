...
import TimbrBranding from './TimbrBranding';
...
class ChatWindow extends React.Component<Props, State> {
  ...
  async componentDidMount() {
    ...
    if (this.isOnDeprecatedVersion()) {
      console.warn('You are currently on a deprecated version of Timbr.');
  ...
  postMessageHandlers = (msg: any) => {
    ...
    switch (event) {
      ...
      case 'timbr:plan':
        return this.handleTimbrPlan(payload);
      case 'timbr:ping':
  ...
  isValidCustomer = async (customerId?: string | null) => {
    ...
    try {
      ...
      this.logger.warn('You might be on an older version of Timbr.');
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        {shouldDisplayBranding && <TimbrBranding />}
        <Box
          px={2}
          sx={{
        ...
        <img
          alt="Timbr"
          src="https://timbr.s3.us-east-2.amazonaws.com/timbr-logo.svg"
          width="0"
          height="0"
        />
      </Flex>
