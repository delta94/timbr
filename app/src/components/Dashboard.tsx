...
import {ChatWidget, Timbr} from '@timbr/chat-widget';
// import {Screen} from '../lib/screen'; // For testing
...
const DashboardHtmlHead = ({totalNumUnread}: {totalNumUnread: number}) => {
  const [htmlTitle, setHtmlTitle] = useState('Timbr');

  const toggleNotificationMessage = () => {
    if (totalNumUnread > 0 && htmlTitle.startsWith('Timbr')) {
      setHtmlTitle(
  ...
  return (
    <Helmet defer={false}>
      <title>{totalNumUnread ? htmlTitle : 'Timbr'}</title>
    </Helmet>
  );
};
...
const Dashboard = (props: RouteComponentProps) => {
  ...
  useEffect(() => {
    ...
    if (REACT_APP_SCREEN_ENABLED && currentUser) {
      const {id, email} = currentUser;
    ...
    <Layout>
          ...
          <Box py={3} sx={{flex: 1}}>
            ...
            >
              ...
              >
                ...
                <Menu.Item key="setup">
                  <Link to="/sessions/setup">Set up Screen</Link>
                </Menu.Item>
              </Menu.SubMenu>
              <Menu.Item
                title="Customers"
      ...
      <Layout style={{marginLeft: 220, background: colors.white}}>
        <Switch>
          ...
          <Route path="/sessions/setup" component={InstallingScreen} />
