...
const DEFAULT_IFRAME_URL = 'https://widget.timbr.io';
...
class ChatWidgetContainer extends React.Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    if (!props.accountId) {
      throw new Error('An `accountId` is required to run the Timbr chat!');
    } else if (!isValidUuid(props.accountId)) {
      console.error(
        `The \`accountId\` must be a valid UUID. (Received invalid \`accountId\`: ${props.accountId})`
      );
      console.error(
        `If you're missing a Timbr \`accountId\`, you can get one by signing up for a free account at https://app.timbr.io/register`
      );
  ...
  async componentDidMount() {
    ...
    // whether to display the Timbr branding or not in the chat window
  ...
  handleChatLoaded = () => {
    ...
    this.send('timbr:plan', {plan: subscriptionPlan});
  };
  ...
  handleCacheCustomerId = (payload: any) => {
    ...
    window.dispatchEvent(
      new CustomEvent('timbr:customer:set', {
        detail: customerId,
      })
    );
