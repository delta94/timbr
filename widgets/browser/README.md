# @timbr/browser-widget

Timbr chat widget for browser.

## Install

```bash
npm install --save @timbr/browser-widget
```
...
import ChatWidget from '@timbr/chat-widget';

// You can also import the following in v1.1.0 and above:
// import {ChatWidget, ChatWindow, Timbr} from '@timbr/chat-widget'; 
//
// The `ChatWindow` component allows you to embed the chat however you want,
// and the `Timbr` object provides access to functions that can programmatically
// open/close the chat widget, e.g.:
//
// `Timbr.open()` // => opens the chat widget
// `Timbr.close()` // => closes the chat widget
// `Timbr.toggle()` // => toggles (opens or closes) the chat widget
...
These are the props you can pass into your `<ChatWidget />` React component, or the fields you can specify in your `Timbr.config`:

| Prop | Type | Value | Default |
| :--- | :--- | :----- | :------ |
| **`accountId`** | `string` | Your Timbr account token | N/A |
...
| **`baseUrl`** | `string` | The base URL of your API if you're self-hosting Timbr | https://app.timbr.io |
| **`iframeUrlOverride`** | `string` | An override of the iframe URL we use to render the chat, if you chose to self-host that as well | https://widget.timbr.io |
...
By default, the example widget points at development servers for the [Timbr API](https://github.com/alxshelepenok/timbr) (which runs at http://localhost:4000) and the [Timbr chat window](https://github.com/alxshelepenok/timbr) (which runs at http://localhost:8080). These values are set with the **baseUrl** and **iframeUrlOverride** props respectively. 

If you want to develop against your account in production, you can update these values to the following:
- `baseUrl: http://app.timbr.io`
- `iframeUrlOverride: https://widget.timbr.io`

_(Note that you can also simply remove these props, since the values above are the defaults.)_

You'll also want to update your account token to point to your own account. If you haven't already, create a free account at https://app.timbr.io/register to get started. 
