...
const DEFAULT_BASE_URL = 'https://app.timbr.io';
...
const SESSION_CACHE_KEY = 'timbr:screen:session';
const CUSTOMER_CACHE_KEY = '__TIMBR____CUSTOMER_ID__';
...
class Screen {
  ...
  static init(config: Config): Screen {
    win.Screen = win.Screen || {};

    // TODO: test if this is actually necessary
    if (win.Screen.initialized) {
      console.warn('Screen has already been initialized!');
      console.warn('This may result in some unexpected issues.');
    }

    const instance = new Screen(config);
    win.Screen = instance;
    instance.listen();
  ...
  async listen(): Promise<Screen> {
    try {
      ...
      this.logger.error('[Screen] Error on `listen`:', err);
    }
  ...
  finish(): void {
    if (this.sessionId) {
      this.logger.debug('[Screen] Marking session over...');
      this.finishBrowserSession(this.sessionId);
    }

    if (this.stop) {
      this.logger.debug('[Screen] Stopping...');
      this.stop();
    }

    if (this.unsubscribe) {
      this.logger.debug('[Screen] Unsubscribing visibility handler...');
      this.unsubscribe();
    }

    if (this.socket && this.socket.disconnect) {
      this.logger.debug('[Screen] Disconnecting socket...');
      this.socket.disconnect();
    }

    if (this.channel && this.channel.leave) {
      this.logger.debug('[Screen] Leaving channel...');
      this.channel.leave();
    }
  ...
  cacheCustomerId = (customerId: string) => {
    win.dispatchEvent(
      new CustomEvent('screen:customer:set', {detail: customerId})
    );
  ...
  onConnectionSuccess(sessionId: string) {
    ...
    win.addEventListener('timbr:customer:set', (e: any) => {
